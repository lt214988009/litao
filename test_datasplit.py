...
#def KfoldData(dataSet,S,seed):
#    train=pd.DataFrame()
#    test=pd.DataFrame()
#    for i in range(1,S+1):
#        train1,test1=SplitData(dataSet,S,i,seed)
...
###输出最后的结果
print('MLP TRUE Percent',np.array(score_mlp).mean())   
print('AdaBoost TRUE Percent',np.array(score_ada).mean())
print('RandomForest TRUE Percent',np.array(score_randomf).mean())
print('SVM TRUE Percent',np.array(score_svm).mean())
print('GBRT TRUE Percent',np.array(score_gbrt).mean())
