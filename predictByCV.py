...
def reducedim(X):
    ...
    return X_red
def SplitData(dataSet,S,k,seed):
    n=len(dataSet)
    testSet=[]
    trainSet=[]
    random.seed(seed)
    for i in range(n):
        if random.randint(0,S) == k:
            testSet.append(dataSet.iloc[i,:])
        else:
            trainSet.append(dataSet.iloc[i,:])
    return pd.DataFrame(trainSet),pd.DataFrame(testSet)
def crossTest(x,y,model):
    results=[]
    k_fold=cross_validation.KFold(len(x),n_folds=5)
    for train,test in k_fold:
        height=model.fit(x.iloc[train],y.iloc[train]).predict(x.iloc[test])
        results.append(np.array((abs(height-y.iloc[test])<2)).mean())
...
learning_rate: 学习率           
'''
randomf=RandomForestRegressor(max_depth=8,n_estimators=150,min_samples_leaf=8, min_samples_split=30,random_state=0)
randomfc=RandomForestClassifier(max_depth=10,n_estimators=150,min_samples_leaf=6, min_samples_split=20,random_state=0)
'''
RandomForest参数详解
...
for i in range(0,61):
    l.append(i)
    data_reg=data[data['Group']==i]
    data_reg=data_reg.iloc[:,[0,1,2,3,5,6,7,8]]
    for k in range(5):
        train,test=SplitData(data_reg,S,k,seed)
        ...
        score_mlp.append((abs(height-y_test)<2).sum()/len(abs(height-y_test)<2))
        #Adaboost
        ada.fit(x_train1,y_train)
        height1=ada.predict(x_test1)
        score_ada.append((abs(height1-y_test)<2).sum()/len(abs(height1-y_test)<2))
        ##随机森林预测
        randomf.fit(x_train1,y_train)
        height2=randomf.predict(x_test1)
        score_randomf.append((abs(height2-y_test)<2).sum()/len(abs(height2-y_test)<2))
        ##SVM预测
        svr.fit(x_train1,y_train)
        height3=svr.predict(x_test1)
        score_svm.append((abs(height3-y_test)<2).sum()/len(abs(height3-y_test)<2))
        ##Gradient Boosting Decision Tree预测
        gbrt.fit(x_train1,y_train)
        height4=gbrt.predict(x_test1)
        score_gbrt.append((abs(height4-y_test)<2).sum()/len(abs(height4-y_test)<2))    
    ###输出最后的结果
    l1.append(np.array(score_mlp).mean())   
    l2.append(np.array(score_ada).mean())
    l3.append(np.array(score_randomf).mean())
    l4.append(np.array(score_svm).mean())
    l5.append(np.array(score_gbrt).mean())
isExists=os.path.exists("cross-11.csv")
if isExists:
    os.remove("cross-11.csv")
with open("cross-11.csv", 'w') as fp:
    fp.write(',mlp,ada,randomf,svm,gbrt\n')
    for line in zip(l,l1,l2,l3,l4,l5):
        line = [str(ele) for ele in line]
        fp.write(','.join(line) + '\n')
####利用Sklearn.cross_validation模块来进行交叉验证
I=[]
J=[]
line1=[]
line2=[]
...
#DataFrame.sample(n=None, frac=None, replace=False, weights=None, random_state=None, axis=None)
    ...
    I.append(i)
    print(i)  
    line1.append(crossTest(x,y,randomf))
    line2.append(crossTest(x,y,mlp))
    line3.append(crossTest(x,y,ada))
    line4.append(crossTest(x,y,gbrt))
    line5.append(crossTest(x,y,svr))   
isExists=os.path.exists("cross-10.csv")
if isExists:
    os.remove("cross-10.csv")
with open("cross-10.csv", 'w') as fp:
    fp.write(',random,mlp,ada,gbrt,svr\n')
    for line in zip(I,line1,line2,line3,line4,line5):
        line = [str(ele) for ele in line]
        fp.write(','.join(line) + '\n')
